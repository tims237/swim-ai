...
import { Calendar, ChevronDown, Plus, X, Heart, Moon, Zap, Activity, AlertCircle, CheckCircle } from 'lucide-react'
...
function Biometries({ utilisateur }: { utilisateur: Utilisateur }) {
  ...
  useEffect(() => {
    if (estNageur && monNageurId) void chargerBiometries(monNageurId)
  ...
  const scoreLabel  = scoreRecup === null ? '—' : scoreRecup >= 75 ? 'Bonne forme' : scoreRecup >= 50 ? 'Modéré' : 'Fatigué'
  const scoreCouleur = scoreRecup === null ? '#94A3B8' : scoreRecup >= 75 ? '#10B981' : scoreRecup >= 50 ? '#F59E0B' : '#EF4444'
  ...
  return (
    <div style={{ fontFamily: theme.policeTexte, maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '14px', marginBottom: '20px' }}>
        {/* Score récupération */}
        <Carte style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ fontSize: '28px', fontWeight: 800, color: '#0F172A', fontFamily: theme.policeTitre, lineHeight: 1 }}>
            {scoreRecup ?? '—'}<span style={{ fontSize: '14px', fontWeight: 400, color: '#94A3B8' }}>{scoreRecup ? ' /100' : ''}</span>
          </div>
        ...
        <Carte style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ fontSize: '28px', fontWeight: 800, color: scoreCouleur, fontFamily: theme.policeTitre, lineHeight: 1 }}>
            {scoreLabel}
          </div>
          {derniere?.rpe && (
        ...
        <Carte style={{ padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              ...
              <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#F8FAFC', borderRadius: '8px', borderLeft: `3px solid ${couleur}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <Carte style={{ padding: '20px 24px' }}>
          ...
          {alertes.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {alertes.map((a, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '8px', background: a.ok ? '#F0FDF4' : '#FFF7ED', border: `1px solid ${a.ok ? '#BBF7D0' : '#FED7AA'}` }}>
                  {a.ok ? <CheckCircle size={15} color="#10B981" /> : <AlertCircle size={15} color="#F59E0B" />}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {([
                  ...
                  <div key={key}>
                    ...
                    <input type="number" step={step} min={min} max={max} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} style={champ} />
                  </div>
                ))}
              </div>

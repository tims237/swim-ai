...
function Performances({ utilisateur }: { utilisateur: Utilisateur }) {
  ...
  useEffect(() => {
    if (estNageur && monNageurId) void chargerDonnees(monNageurId)
  ...
  const chargerDonnees = async (id: number) => {
    try {
      const [s, p] = await Promise.all([getSessionsNageur(id), getPerformancesNageur(id)])
      setSessions(s)
      setPerformances(p)
    } catch { /* silencieux */ }
  }
  ...
  // KPI depuis données réelles
  const perfsAvecChrono = performances.filter(p => p.temps_s)
  const meilleurChrono  = perfsAvecChrono.length ? Math.min(...perfsAvecChrono.map(p => p.temps_s!)) : null
  const dernierChrono   = perfsAvecChrono[0]?.temps_s ?? null
  const progression     = meilleurChrono && dernierChrono
  ...
  // Meilleures perfs par distance
  const meilleurParDistance = DISTANCES.reduce((acc, d) => {
    const perfsD = performances.filter(p => p.distance_m === d && p.temps_s)
    if (perfsD.length) acc[d] = Math.min(...perfsD.map(p => p.temps_s!))
    return acc
    ...
    <div style={{ fontFamily: theme.policeTexte, maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '14px', marginBottom: '20px' }}>
        {[
          { label: 'Meilleur chrono', Icon: Timer,       accent: theme.primary,
            ...
            sous: dernierChrono && meilleurChrono && dernierChrono !== meilleurChrono ? `↓ ${Math.abs(dernierChrono - meilleurChrono).toFixed(2)}s vs saison` : 'Aucune donnée' },
          { label: 'Progression 4 sem.', Icon: progression !== null && progression < 0 ? TrendingDown : TrendingUp,
            accent: progression !== null && progression < 0 ? '#10B981' : '#F59E0B',
            valeur: progression !== null ? `${progression > 0 ? '+' : ''}${progression}%` : '—',
            sous: progression !== null ? (progression < 0 ? 'Amélioration' : 'À améliorer') : 'Aucune donnée' },
          { label: 'Vitesse moyenne', Icon: Zap, accent: theme.secondary,
            valeur: vitesseMoy ? `${vitesseMoy} m/s` : '—',
            sous: 'Moyenne toutes distances' },
          { label: 'Objectif saison', Icon: Target, accent: '#10B981',
            valeur: meilleurChrono ? `${(meilleurChrono * 0.98).toFixed(2)}s` : '—',
            sous: meilleurChrono ? `Écart : ${(meilleurChrono * 0.02).toFixed(2)}s` : 'Aucune donnée' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '14px', alignItems: 'start' }}>
        ...
        <Carte style={{ overflow: 'hidden' }}>
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #F1F5F9' }}>
            <SectionTitre>Évolution des chronos</SectionTitre>
          </div>
            ...
            <tbody>
              {performances.length === 0 ? (
                ...
                <tr key={p.id} style={{ borderTop: '1px solid #F1F5F9', background: i % 2 === 0 ? '#FFFFFF' : '#FAFAFA' }}>
                  ...
                  <td style={{ padding: '13px 16px', fontSize: '13px', color: '#475569' }}>{p.distance_m ? `${p.distance_m}m` : '—'}</td>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    {p.style_nage ? (
                      <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 600, background: COULEURS_STYLE[p.style_nage].bg, color: COULEURS_STYLE[p.style_nage].text }}>
                        {p.style_nage}
                      </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <Carte style={{ padding: '20px 24px' }}>
            ...
            {performances.filter(p => p.temps_s && p.distance_m).length === 0 ? (
              <p style={{ color: '#94A3B8', fontSize: '13px', textAlign: 'center', padding: '16px 0' }}>Aucune performance</p>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Distance', 'Chrono', 'Style'].map(h => (
                      <th key={h} style={{ padding: '6px 8px', textAlign: 'left', fontSize: '10px', color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {performances.filter(p => p.temps_s && p.distance_m).slice(0, 5).map((p, i) => (
                    <tr key={p.id} style={{ borderTop: i === 0 ? 'none' : '1px solid #F1F5F9' }}>
                      <td style={{ padding: '10px 8px', fontSize: '13px', color: '#475569' }}>{p.distance_m}m</td>
                      <td style={{ padding: '10px 8px', fontSize: '14px', fontWeight: 800, color: theme.primary, fontFamily: theme.policeTitre }}>{p.temps_s}s</td>
                      <td style={{ padding: '10px 8px' }}>
                        {p.style_nage ? (
                          <span style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 600, background: COULEURS_STYLE[p.style_nage].bg, color: COULEURS_STYLE[p.style_nage].text }}>
                            {p.style_nage}
                          </span>
                        ) : '—'}
                      </td>
          ...
          <Carte style={{ padding: '20px 24px' }}>
            ...
            {Object.keys(meilleurParDistance).length === 0 ? (
              <p style={{ color: '#94A3B8', fontSize: '13px', textAlign: 'center', padding: '16px 0' }}>Aucune donnée</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {Object.entries(meilleurParDistance).map(([dist, chrono]) => (
                  <div key={dist} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#F8FAFC', borderRadius: '8px', borderLeft: `3px solid ${theme.primary}` }}>
                    <div>
                      <div style={{ fontSize: '11px', color: '#94A3B8', marginBottom: '2px' }}>{dist}m</div>
                      <div style={{ fontSize: '18px', fontWeight: 800, color: theme.primary, fontFamily: theme.policeTitre }}>{chrono}s</div>
                    </div>
                    <span style={{ fontSize: '11px', color: '#94A3B8' }}>meilleur</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {formOuvert && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }} onClick={() => setFormOuvert(false)}>
          <div style={{ background: '#FFFFFF', borderRadius: '16px', padding: '28px 32px', width: '480px', maxWidth: '90vw', boxShadow: '0 20px 60px rgba(15,23,42,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontFamily: theme.policeTitre, fontSize: '18px', fontWeight: 700, color: '#0F172A', margin: 0 }}>Nouvelle performance</h2>

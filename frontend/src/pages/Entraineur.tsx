...
function Entraineur() {
  ...
  const StatCard = ({ titre, valeur, sousTitre, couleur }: { titre: string; valeur: number; sousTitre: string; couleur: string }) => (
  ...
  return (
    <div style={{ fontFamily: theme.policeTexte }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
        <h1 style={{ fontFamily: theme.policeTitre, color: theme.neutral, margin: 0, fontSize: '28px', fontWeight: 800 }}>Vue Entraîneur</h1>
      ...
      <p style={{ color: theme.texteDoux, marginBottom: '24px', fontSize: '15px' }}>Pilotage collectif et décisions coach</p>

      {loading && <p style={{ color: theme.texteDoux }}>Chargement des données équipe...</p>}
            ...
            <div style={{ ...carte, padding: 0, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: theme.tertiary }}>
                  <tr>
                    {['Nageur', 'Spécialité', 'Dernier chrono', 'Progression', 'Fatigue', 'ACWR', 'Recommandation'].map((h) => (
          ...
          {nageurSelectionne && dashboards[nageurSelectionne] && (
            <div style={{ ...carte, marginTop: '24px', padding: '24px' }}>
              <h3 style={{ marginBottom: '16px', color: theme.neutral, fontFamily: theme.policeTitre, fontSize: '18px' }}>
                💡 Recommandations — {nageurs.find((n) => n.id === nageurSelectionne)?.prenom} {nageurs.find((n) => n.id === nageurSelectionne)?.nom}
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {dashboards[nageurSelectionne]!.recommandations.map((rec, i) => (
                  <div key={i} style={{ padding: '12px 16px', borderRadius: '10px', background: theme.tertiary, border: `1px solid ${theme.bordure}`, fontSize: '14px', color: theme.texte }}>
                    {rec}
                  </div>
                ))}
              </div>

...
interface ExportEntry { nom: string; date: string; format: 'CSV' | 'JSON'; statut: 'Complété' }
...
function Sessions({ utilisateur }: { utilisateur: Utilisateur }) {
  ...
  const [formatExport,      setFormatExport]      = useState<'CSV' | 'JSON'>('CSV')
  ...
  // Form modification — pré-rempli depuis la session cliquée
  ...
  const ouvrirModification = (s: Session) => {
    ...
    setFormEdit({
      ...
      duree_min:   s.duree_min ? String(s.duree_min) : '',
    })
  }
  ...
  const genererRapport = () => {
    ...
    if (formatExport === 'CSV') {
      ...
      const lignes = sessionsAffichees
        ...
        .join('\n')
      nomFichier = `Sessions_${typeRapport.replace(/ /g, '_')}_${now}.csv`
      ...
      nomFichier = `Sessions_${typeRapport.replace(/ /g, '_')}_${now}.json`
      telecharger(
    ...
    setHistoriqueExports(prev => [entry, ...prev.slice(0, 4)])
  }
    ...
    <div style={{ fontFamily: theme.policeTexte, maxWidth: '1100px', margin: '0 auto' }}>
                ...
                <td style={{ padding: '13px 16px' }}>
                  {s.type_seance ? (
                    <span style={{
                      ...
                      background: COULEURS_TYPE[s.type_seance].bg,
                      color:      COULEURS_TYPE[s.type_seance].text,
                    }}>
                      {s.type_seance}
                    </span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '20px', alignItems: 'start' }}>
                ...
                {rapportsRecents.map((r, i) => (
                  <div key={i} style={{ border: '1px solid #E2E8F0', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{
                        background: r.format === 'CSV' ? '#EFF6FF' : '#F0FDF4',
                        color:      r.format === 'CSV' ? theme.primary : '#065F46',
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#0F172A', lineHeight: 1.3 }}>{r.nom}</div>
                    <div style={{ fontSize: '12px', color: '#64748B' }}>{sessionsAffichees.length} sessions · {totalMinutes} min</div>
                    <button onClick={genererRapport} style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                      padding: '8px', border: '1px solid #E2E8F0', borderRadius: '8px',
                      background: '#FFFFFF', color: '#475569', cursor: 'pointer', fontSize: '13px', marginTop: '4px',
                    }}>
              ...
              <thead style={{ background: '#F8FAFC' }}>
                <tr>
                  {['Nom du Fichier', 'Date', 'Format', 'Statut'].map(h => (
                    <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '11px', color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      {h}
                    </th>
                  ))}
                </tr>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <Carte style={{ padding: '20px 24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Type de rapport
                </label>
              ...
              <div>
                <label style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', display: 'block', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Format d'export
                </label>
      ...
      {formOuvert && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
          ...
          <div style={{ background: '#FFFFFF', borderRadius: '16px', padding: '28px 32px', width: '440px', maxWidth: '90vw', boxShadow: '0 20px 60px rgba(15,23,42,0.2)' }}
      ...
      {sessionAModifier && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
          onClick={() => setSessionAModifier(null)}>
          <div style={{ background: '#FFFFFF', borderRadius: '16px', padding: '28px 32px', width: '440px', maxWidth: '90vw', boxShadow: '0 20px 60px rgba(15,23,42,0.2)' }}
            onClick={e => e.stopPropagation()}>
